<template>
  <ElDrawer v-model="visible" title="代付订单详情" size="680px" @open="handleOpen">
    <ElDescriptions v-loading="loading" :column="1" border>
      <ElDescriptionsItem label="平台代付单号">{{ detail.withdraw_no }}</ElDescriptionsItem>
      <ElDescriptionsItem label="商户代付单号">{{ detail.out_biz_no || '-' }}</ElDescriptionsItem>
      <ElDescriptionsItem label="代付金额">{{ detail.amount }} 元</ElDescriptionsItem>
      ...
      <ElDescriptionsItem label="上游代付单号">{{ detail.transfer_no || '-' }}</ElDescriptionsItem>
      <ElDescriptionsItem label="下游通知地址">{{ detail.notify_url || '-' }}</ElDescriptionsItem>
      ...
      <ElDescriptionsItem label="进单时间">{{ detail.create_time || '-' }}</ElDescriptionsItem>
    </ElDescriptions>
    ...
    <ElAlert
      ...
      :closable="false"
      show-icon
      title="该代付单无银行卡快照（多为历史单据），无法展示进单时收款信息。"
    />
    <!-- 收款银行卡：仅展示进单时落库快照 -->
    <ElCard class="bank-card-panel" shadow="never">
      <template #header>
        <div class="bank-card-panel__title">
          ...
          <span>收款银行卡</span>
          <ElTag v-if="detail.account_no" size="small" type="info">进单快照</ElTag>
        </div>
      </template>
